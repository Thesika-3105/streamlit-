...
def load_data():
    ...
    nco_df.columns = [c.strip().replace(" ", "_") for c in nco_df.columns]
    scheme_df.columns = [c.strip().replace(" ", "_") for c in scheme_df.columns]
...
if query:
    ...
    # Translate job titles safely
    results_display = results.copy()
    results_display['Job_Title'] = results_display['Job_Title'].apply(lambda x: translate_text(str(x), lang))
    ...
    schemes_display['Scheme_Name'] = schemes_display['Scheme_Name'].apply(lambda x: translate_text(str(x), lang))
